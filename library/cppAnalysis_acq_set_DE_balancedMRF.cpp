...
void DE_isochromat_efficiency(double *eff, double *signal_real, double *signal_imag, double *N, double *theta, double *RF, double *TR0, double *T1, double *T2, double *M0, double *B0, double *P0)
{
    ...
    for (int j=0; j <(length*Nloops); j++){
        ...
        if (j>=(Nloops-1)*length){
            ...
            signal_real[j%length]    = exp(-R2 * TE(j%length)) * Mminus(0);
            signal_imag[j%length]    = exp(-R2 * TE(j%length)) * Mminus(1);
        }
    ...
    eff[0] = T1[0] / pow(V(0,0) * Tacq * 1e-3, 0.5);
    eff[1] = T2[0] / pow(V(1,1) * Tacq * 1e-3, 0.5);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    DE_isochromat_efficiency(eff, signal_real, signal_imag, N, theta, RF, TR, T1, T2, M0, B0, P0);
