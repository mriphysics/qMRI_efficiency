...
void isochromat_efficiency(double *f, double *N, double *theta, double *RF, double *TR0, double *T1, double *T2, double *M0, double *B0, double *P0)
{
	...
	int length = static_cast<int>(N[0]);
    ...
    for (int j=0; j<length; j++){
        ...
        update_dRz(FPangle + P0[0], 2*PI*TE(j)*pow(10.0,-3.0) );       
    ...
    V = F.inverse();
    f[0] = (V(0,0) * R1 * R1 + V(1,1) * R2 * R2) * Tacq;
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    isochromat_efficiency(f, N, theta, RF, TR, T1, T2, M0, B0, P0);
