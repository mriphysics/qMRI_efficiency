...
void DE_EPG_GRE_efficiency(double *eff, double *signal_real, double *signal_imag, double *N, double *FA, double *RF, double *TR, double *TE, double *T1, double *T2)
{
    int Nloops = 50;
    int Nrep = (int) N[0];
    int n = Nrep * Nloops;
    ...
    for (int j=0; j<n; j++){
        ...
        E1 = exp(-TR[(j%Nrep)] / T1[0]);
        E2 = exp(-TR[(j%Nrep)] / T2[0]);
        E2_TE = exp(-TE[(j%Nrep)] / T2[0]);
        dEdT1 = TR[(j%Nrep)] * E1 / pow(T1[0],2);
        dEdT2 = TR[(j%Nrep)] * E2 / pow(T2[0],2);
        dEdT2_TE = TE[(j%Nrep)] * E2_TE / pow(T2[0],2);

        //apply T
        update_T(FA[(j%Nrep)], RF[(j%Nrep)]);
        ...
        if(j>=(n-Nrep))
        {
            ...
            add2FIM(); 
            signal_real[j%Nrep] = prev_Fminus(0).real();      
            signal_imag[j%Nrep] = prev_Fminus(0).imag();
...
void mexFunction(int nlhs, mxArray *plhs[], int rhs, const mxArray *prhs[])
{
    ...
    DE_EPG_GRE_efficiency(eff, signal_real, signal_imag, N, FA, RF, TR, TE, T1, T2);
