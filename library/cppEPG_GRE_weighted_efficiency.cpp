...
void EPG_GRE_weighted_efficiency(double *f, double *N, double *w, double *FA, double *RF, double *TR, double *TE, double *T1, double *T2)
{
    int n = (int) N[0];
    ...
    double eta_T1 = T1[0] / pow(V(0,0) * Tacq, 0.5); 
    double eta_T2 = T2[0] / pow(V(1,1) * Tacq, 0.5); 
...
void mexFunction(int nlhs, mxArray *plhs[], int rhs, const mxArray *prhs[])
{
    ...
    EPG_GRE_weighted_efficiency(f, N, w, FA, RF, TR, TE, T1, T2);
