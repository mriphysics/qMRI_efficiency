...
void DE_EPG_GRE_eff(double *f, double *N, double *FA, double *RF, double *TR, double *TE, double *T1, double *T2)
{
    int Nrep = (int) N[0];

    double totalTime = 0.0;
    for(int j=0; j<Nrep; j++) totalTime += TR[j];
    int Nloops = (int) (10.0 * T1[0] / totalTime);
    ...
    V = F.inverse();
    //V = F.completeOrthogonalDecomposition().pseudoInverse();
...
void mexFunction(int nlhs, mxArray *plhs[], int rhs, const mxArray *prhs[])
{
    ...
    DE_EPG_GRE_eff(f, N, FA, RF, TR, TE, T1, T2);
