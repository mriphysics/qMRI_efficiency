%% Single-start optimisation of DE spoiled MRF acquisition settings
...
opt_param_DE_spoiledMRF_ss.N        = zeros(length(N), 1);
opt_param_DE_spoiledMRF_ss.costFunc = zeros(length(N), 1);
opt_param_DE_spoiledMRF_ss.acqSet   = cell(length(N), 1);
...
for nn=1:length(N)
    ...
    opt_param_DE_spoiledMRF_ss.N(nn) = N(nn);
    ...
    % set cost function
    func = @(u) cppDE_EPG_GRE_efficiency(N(nn), u(1:N(nn)), RF0, u(N(nn)+1:end), TE0, T1, T2);

    u0 = [FA0(1:N(nn)); TR0(1:N(nn))];
    
    tic
    [uopt,fval] = fmincon(func, u0, [], [], [], [], lb, ub, [], options);

    opt_param_DE_spoiledMRF_ss.costFunc(nn) = fval;
    
    % save optimal acquisition settings
    opt_param_DE_spoiledMRF_ss.acqSet{nn}.FAopt = uopt(1:N(nn));
    opt_param_DE_spoiledMRF_ss.acqSet{nn}.TRopt = uopt(N(nn)+1:2*N(nn));
...
save('opt_param_DE_spoiledMRF_ss','opt_param_DE_spoiledMRF_ss')
