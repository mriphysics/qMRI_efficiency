%% Multi-start optimization of DE spoiled MRF acquisition settings
...
opt_param_DE_spoiledMRF_ms.N        = zeros(length(N), 1);
opt_param_DE_spoiledMRF_ms.costFunc = zeros(length(N), 1);
opt_param_DE_spoiledMRF_ms.acqSet   = cell(length(N), 1);
...
for nn=1:length(N)
    ...
    opt_param_DE_spoiledMRF_ms.N(nn) = N(nn);
    ...
    % set cost function
    func = @(u) cppDE_EPG_GRE_efficiency(N(nn), u(1:N(nn)), RF0, u(N(nn)+1:end), TE0, P(1), P(2));
    ...
    opt_param_DE_spoiledMRF_ms.costFunc(nn) = all_fval(idx_best);
    
    % save optimal acquisition settings
    opt_param_DE_spoiledMRF_ms.acqSet{nn}.FAopt = uopt(1:N(nn));
    opt_param_DE_spoiledMRF_ms.acqSet{nn}.TRopt = uopt(N(nn)+1:2*N(nn));
...
save('opt_param_DE_spoiledMRF_ms','opt_param_DE_spoiledMRF_ms')
