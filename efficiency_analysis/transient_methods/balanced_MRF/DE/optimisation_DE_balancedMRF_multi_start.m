%% Multi-start optimisation of DE balanced MRF acquisition settings 
...
% comment lines 51-55 and replace parfor loop in line 104 by a for loop
...
opt_param_DE_balancedMRF_ms.N        = zeros(length(N), 1);
opt_param_DE_balancedMRF_ms.costFunc = zeros(length(N), 1);
opt_param_DE_balancedMRF_ms.acqSet   = cell(length(N), 1);
...
for nn=1:length(N)
    ...
    opt_param_DE_balancedMRF_ms.N(nn) = N(nn);
    ...
    opt_param_DE_balancedMRF_ms.costFunc(nn) = all_fval(idx_best);
    
    % save optimal acquisition settings
    opt_param_DE_balancedMRF_ms.acqSet{nn}.FAopt = theta2alpha(uopt);
...
save('opt_param_DE_balancedMRF_ms','opt_param_DE_balancedMRF_ms')
