%% Single-start optimisation of non-DE balanced MRF acquisition settings
...
N = [500 600 700 800 900 1000 1200 1400 1600]; 

% structure to save optimisation results
opt_param_nonDE_balancedMRF_ss.N        = zeros(length(N), 1);
opt_param_nonDE_balancedMRF_ss.costFunc = zeros(length(N), 1);
opt_param_nonDE_balancedMRF_ss.acqSet   = cell(length(N), 1);
...
for nn=1:length(N)
    ...
    opt_param_nonDE_balancedMRF_ss.N(nn) = N(nn);
    ...
    opt_param_nonDE_balancedMRF_ss.costFunc(nn) = fval;
    
    % save optimal acquisition settings
    opt_param_nonDE_balancedMRF_ss.acqSet{nn}.FAopt = theta2alpha(uopt);
...
save('opt_param_nonDE_balancedMRF_ss','opt_param_nonDE_balancedMRF_ss')
